...
cd /RSM
rm -rf /RSM/mux && git clone https://github.com/JaredStemper/Scylla.git
cp /RSM/Scylla/tmux.conf ~/.tmux.conf
...
(crontab -l ; echo "0,15,30,45 * * * * /bin/bash /RSM/Scylla/tmuxSessionHistoryCapture.sh") | crontab -

cd /RSM; clear

python3 /RSM/Scylla/prefillTest.py "tmuxinator start -p /RSM/Scylla/tmuxinator/internalTemplate-initScan.yml msfWorkspace=CLIENTNAME domain=domain.local nessusKey=NESSUSKEY nomadPass='nomadPass'"
